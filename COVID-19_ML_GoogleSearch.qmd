...
Let's look at some of the Google Search Trends variability along with new cases in Pennsylvania. It is clearly seen that, some parts of the trends are cosistent with the infection rate (i.e. search trends for hypoxemia in early 2022). However, most of the time they are noisy and weakly correlated to the infections.
...
plot_df <- covid19_ml %>%
  ...
  mutate(
    # Normalize everything to 0-1 scale for plotting
  ...
  mutate(
    trend_norm = scales::rescale(trend_value)
  )
